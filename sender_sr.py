"""
-------------------------------------------------------
Assignment 02 BONUS, Sender Selective Repeat
-------------------------------------------------------
Author:  Group 30
...
"""
    Sends Sequence Of Packets Using Selective Repeat Approach
    Uses One Timer For The Oldest Unacknowledged Packet

"""
def send_sr(packets:dict, final_seq:int, window:int, timeout:float, max_attempts:int=20):
    ...
    acknowledged = {seq: False for seq in packets.keys()}
    ...
    while base <= final_seq:
        ...
        #No Packet Recieved Within Timeout Period: Resend Base Packet
        if not recieved_packet:
            if base <= final_seq and not acknowledged[base]:
                print(f"[Sender]: Resending Packet #{base} (base={base})")
                sock.sendto(packets[base], destination)
                retransmissions += 1
            base_sent = time.time()
            attempts += 1
        ...
        #Packet Recieved
            ...
            #Correct ACK
            if packet_type == ACK_TYPE and recieved_ack in acknowledged:
                print(f"[Sender]: Recieved ACK{recieved_ack}#")
                acknowledged[recieved_ack] = True

                #Slide Window Past Acknowledged Packets
                while base <= final_seq and acknowledged[base]:
                    base += 1
...
"""
Sends A File Or Specified Bytes Using Selective Repeat
"""
def main():
    global destination
    global sock
    ...
    try:
        ...
        #Send Packets
        send_sr(packets, final_seq, args.window, args.timeout)
